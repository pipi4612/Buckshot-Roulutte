...
# --- A 方案 path guard：確保優先載入本專案(611_project)的 engine / items ---
import os, sys
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
if THIS_DIR not in sys.path:
    sys.path.insert(0, THIS_DIR)

# 若同一工作階段曾載入別專案的同名套件，清掉避免衝突
...
# ===============================================================
# ⚙️ Spyder 友善設定（不需下指令就能預覽一局）
# ===============================================================
# 在 Spyder 執行時，多局統計前是否自動示範一局？
AUTO_PREVIEW_FOR_SPYDER = True
# 自動示範用的道具（None 代表沿用 args.item）
AUTO_PREVIEW_ITEM = None
# 自動示範時是否顯示實彈位置（教學/除錯用）
AUTO_PREVIEW_BULLETS = True
# 自動示範時是否使用逐字動畫
AUTO_PREVIEW_ANIM = True
...
def _running_in_spyder() -> bool:
    ...
    if "spyder_kernels" in sys.modules:
        return True
    # 有些環境會掛環境變數
    for key in ("SPYDER_ARGS", "SPYDER_PARENT_DIR"):
...
WEIGHTED_WEIGHTS = [0.10, 0.10, 0.10, 0.20, 0.20, 0.20, 0.10]
...
def run_many_games(trials=10000, item="weighted", show_progress=False, quiet=False, **kwargs):
    ...
    參數：
      ...
      - quiet: True 時不印起始提示（預設 False 會提示）
...
# ===============================================================
# 命令列參數（保留，讓你有需要時也能下指令用）
# ===============================================================
def _build_arg_parser():
    p = argparse.ArgumentParser(description="611 對局：單局 / 多局統計（含權重抽選）")
    p.add_argument("--mode", choices=["single", "many"], default="many",
                   help="運行模式：single=單局；many=多局統計（預設 many）")
    p.add_argument("--item", default="weighted",
                   help="道具：weighted/random/或指定名稱（converter/cigarette/saw/beer/handcuff/magnifier/phone）")
    p.add_argument("--trials", type=int, default=10000, help="多局統計試次（預設 10000）")
    p.add_argument("--bullets", action="store_true", help="單局時顯示實彈位置")
    p.add_argument("--no-anim", action="store_true", help="單局時關閉逐字動畫")
    p.add_argument("--quiet", action="store_true", help="many 模式不輸出起始提示")
    p.add_argument("--progress", action="store_true", help="many 顯示每 1000 局進度")

    # 保留可選的預覽旗標（若有天要在命令列使用）
    p.add_argument("--preview-first", action="store_true",
                   help="many 模式開始前先示範一局完整流程（有逐回合輸出）")
    p.add_argument("--preview-item", default=None,
                   help="示範單局要用的道具（預設沿用 --item）")
    p.add_argument("--preview-bullets", action="store_true",
                   help="示範單局顯示實彈位置")
    return p
...
if __name__ == "__main__":
    ...
    do_preview = args.preview_first or (AUTO_PREVIEW_FOR_SPYDER and in_spyder)

    if args.mode == "single":
        # 單局：預設顯示過程；可用 --bullets 顯示彈位；--no-anim 關動畫
        run_one_game(
            item=args.item,
            show_text=True,
        ...
        # 先示範一局（在 Spyder 會自動啟用，除非你把 AUTO_PREVIEW_FOR_SPYDER=False）
        if do_preview:
            preview_item = args.preview_item or AUTO_PREVIEW_ITEM or args.item
            run_one_game(
                item=preview_item,
                show_text=True,
                show_bullets=(args.preview_bullets or AUTO_PREVIEW_BULLETS),
                anim=(not args.no_anim if args.preview_first else AUTO_PREVIEW_ANIM),
            )
            print("\n—— 示範結束，開始多局統計 ——\n")

        # 多局統計：預設 10000 次權重抽選，印起始提示與最終統計
        run_many_games(
            trials=args.trials,
            item=args.item,
